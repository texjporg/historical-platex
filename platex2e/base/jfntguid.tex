...
% is distributed together with all files mentioned in manifest.txt.
% 
% If you receive only some of these files from someone, complain!
% 
% Permission granted to copy this file to another file with a
...
\LaTeX{}の修正については、
\texttt{cfgguide.tex}の\emph{\cfgguide}で説明されています。
\LaTeX{}を修正する上での我々のポリシーは、
\texttt{modguide.tex}の\emph{\modguide}で説明されています。
...
\m{fontarg}に要求されたサイズを続けた外部フォントを生成します。
たとえば、|<<10.98>> genb * dctt|は|dctt1098|を生成します。

\item[sgenb]
\NEWfeature{1995/12/01}
`genb'と同じですが、端末に警告を出力せず、ログにだけ出力します。
...
\TeX{}は、テキストの大文字/小文字を変換するために|\uppercase|と|\lowercase|の
二つのプリミティブを提供しています。しかし、これらの\TeX{}プリミティブは、
|\ae|や|\aa|のようなコマンドによってアクセスすることができません。
この問題を解決するために、\LaTeX{}は、上記の二つのコマンドを提供します。

最終的には、|\MakeUppercase|のようなコマンドよりも、
すべてが大文字のフォントを使いたいのですが、現時点では不可能です。
なぜなら、そのようなフォントが存在しないからです。

詳細については、\texttt{clsguide.tex}を参照してください。

\NEWdescription{1995/12/01}
大文字/小文字の規則が適切に動作し、現在のハイフネーションを提供できれば、
\LaTeXe{}は、文書のいたるところで、固定された変換用テーブルと同じものを使う
\emph{べき}です。
そのテーブルは、|T1|エンコードフォント用にデザインされたテーブルです。
これは、すべてのラテンアルファベットの標準\TeX{}フォントでうまく動きます。
しかし、その他のアルファベットを使うとき、問題を起こします。
